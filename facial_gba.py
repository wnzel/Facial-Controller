# facial_gba.py — Facial gestures -> Keyboard (mGBA-friendly)
# Mappings:
...
# ---- B (mouth/jaw open) with hysteresis (tap on release) ----
B_ON_TH     = 0.35     # start detecting mouth open
B_OFF_TH    = 0.20     # release threshold (hysteresis)
B_MIN_SEC   = 0.10     # must be open at least this long

# ---- ENTER (long blink: both eyes closed) ----
BLINK_ON_TH     = 0.40   # eyeBlinkLeft/Right > this => closed
BLINK_OFF_TH    = 0.20   # open hysteresis (about 45% of ON)
ENTER_MIN_SEC   = 0.30   # how long both closed before ENTER
ENTER_COOLDOWN  = 0.50   # gap between ENTER taps
...
def main():
    ...
    open_start = None                   # B (mouth/jaw) open timer
    both_start = None                   # ENTER (both eyes closed) timer
    last_tap = 0.0
    last_enter = 0.0
    sent_msg, sent_t = "", 0.0
    ...
    # smoothed signals
    s = {k:None for k in [
        "mouthOpen","jawOpen","open","smile","brow","afrown",
    ...
    while True:
        ...
        jawOpen   = get_bs("jawOpen",  bs)   # <— extra cue for reliability
        ...
        blinkL    = get_bs("eyeBlinkLeft",  bs)
        blinkR    = get_bs("eyeBlinkRight", bs)
        ...
        # Robust mouth/jaw open signal (for B)
        open_raw  = max(mouthOpen, 0.8*jawOpen)

        # ---- smooth ----
        s["mouthOpen"] = ema(s["mouthOpen"], mouthOpen)
        s["jawOpen"]   = ema(s["jawOpen"],   jawOpen)
        s["open"]      = ema(s["open"],      open_raw)     # combined mouth/jaw
        ...
        s["blinkL"]    = ema(s["blinkL"],    blinkL)
        s["blinkR"]    = ema(s["blinkR"],    blinkR)
        ...
        bL = s["blinkL"] or 0.0
        bR = s["blinkR"] or 0.0
        both_closed = (bL > BLINK_ON_TH) and (bR > BLINK_ON_TH)
        both_open   = (bL < BLINK_OFF_TH) and (bR < BLINK_OFF_TH)  # hysteresis

        if both_closed:
            if both_start is None:
                both_start = now
        else:
            if both_start is not None and both_open:
                dur = now - both_start
                both_start = None
                if dur >= ENTER_MIN_SEC and (now - last_enter) > ENTER_COOLDOWN:
                    tap(TAP_KEYS["ENTER"]); last_enter = now
                    sent_msg, sent_t = "ENTER (both eyes closed)", now
        # ===================================================================
        ...
        # ---------------- A: teeth-frown (tap on release) ----------------------------
        af = s["afrown"] or 0.0
        can_arm_af = (s["smile"] or 0.0) < SMILE_BLOCK_TH and (s["open"] or 0.0) < OPEN_BLOCK_TH
        ...
        if op >= B_ON_TH:
            ...
            if open_start is not None and op <= B_OFF_TH:
                dur = now - open_start
                open_start = None
                if dur >= B_MIN_SEC and (now - last_tap) > DEBOUNCE_TAP_SEC:
                    tap(TAP_KEYS["B"]); last_tap = now
                    sent_msg, sent_t = "B (mouth/jaw open)", now
